...
DEBUG = getattr(properties, 'debug', False)
TEMPLATE_DEBUG = getattr(properties, 'template_debug', DEBUG)
SITE_ROOT = getattr(properties, 'site_root', 'http://localhost:8000/')
APP_ROOT = getattr(properties, 'app_root','.')
MANAGERS = getattr(properties, 'admins',( ('aaloy','aaloy@apsl.net'),))

DATABASE_ENGINE = getattr(properties,'database_engine', 'sqlite3')
DATABASE_NAME = getattr(properties,'database_name','db')
DATABASE_USER = getattr(properties,'database_user','')
...
MEDIA_ROOT = getattr(properties, 'media_root', APP_ROOT+r'/media/')
...
SECRET_KEY = getattr(properties, 'secret_key', 'default-secret-key-o-yeah!')
...
CACHE_BACKEND=getattr(properties, 'cache_backend', 'file:///tmp/jq_agenda_cache')
CACHE_MIDDLEWARE_KEY_PREFIX=getattr(properties, 'cache_prefix', 'app_fuse')
CACHE_MIDDLEWARE_SECONDS=getattr(properties, 'cache_seconds', 3600)
SESSION_ENGINE=getattr(properties, 'session_engine', 'django.contrib.sessions.backends.cache')
